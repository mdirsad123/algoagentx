...
class StrategyDeployment(Base):
    ...
    runner_interval_mode = Column(String(32), nullable=False, server_default="CANDLE_CLOSE")
    broker_delay_seconds = Column(Integer, nullable=False, server_default="1")
    missed_candle_retry_seconds = Column(Integer, nullable=False, server_default="1")
...
class LiveOrder(Base):
    ...
    requested_lot = Column(Numeric(18, 8), nullable=True)
    final_lot = Column(Numeric(18, 8), nullable=True)
    requested_quantity = Column(Numeric(18, 8), nullable=True)
    final_quantity = Column(Numeric(18, 8), nullable=True)
    risk_amount = Column(Numeric(18, 4), nullable=True)
    actual_risk = Column(Numeric(18, 4), nullable=True)
    instrument_spec_snapshot = Column(JSONB, nullable=True)
    runtime_config_snapshot = Column(JSONB, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
...
class PlatformTradingSettings(Base):
    ...
    min_broker_sync_interval_seconds = Column(Integer, nullable=False, server_default="1")
