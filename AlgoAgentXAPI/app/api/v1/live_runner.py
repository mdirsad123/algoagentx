...
async def _set_auto_runner(db: AsyncSession, deployment: StrategyDeployment, enabled: bool) -> StrategyDeployment:
    ...
    db.add(LiveTradeLog(
        ...
        metadata_json={"auto_runner_enabled": enabled},
    ))
    await db.commit()
...
async def disable_auto_runner(deployment_id: UUID, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    return success_response({"deployment_id": str(row.id), "auto_runner_enabled": row.auto_runner_enabled, "last_runner_at": row.last_runner_at, "last_processed_candle_time": row.last_processed_candle_time, "runner_error_count": row.runner_error_count, "runner_last_error": row.runner_last_error}, "Auto runner disabled")
