...
function AttachmentList({ attachments }: { attachments?: SupportAttachment[] }) {
  ...
  const handleDownload = async (attachment: SupportAttachment) => {
    try {
      setDownloadingId(attachment.id);
      await supportApi.downloadAttachment(attachment);
    } catch (error: any) {
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      {attachments.map((attachment) => (
        <button
          ...
          className="inline-flex max-w-full items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-left text-xs text-purple-100/85 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {downloadingId === attachment.id ? (
            <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-fuchsia-200" />
          ) : (
              ...
              <div className="max-h-[720px] space-y-3 overflow-y-auto pr-1">
                {orderedTickets.map((ticket) => (
                  <button
                    ...
                    className={cx(
                      "w-full rounded-2xl border p-4 text-left transition hover:bg-white/10",
                      selectedTicket?.id === ticket.id ? "border-fuchsia-400/40 bg-fuchsia-500/15" : "border-white/10 bg-white/5"
                    )}
                  >
