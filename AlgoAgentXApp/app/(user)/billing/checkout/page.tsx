...
function BillingCheckoutContent() {
  ...
  const packCode = normalizeUpper(params.get("pack"));
  const initialCoupon = normalizeUpper(params.get("coupon"));
  ...
  const buildPayload = useCallback(
    (coupon?: string | null, method: PaymentMethod = paymentMethod) => ({
      ...
      credit_amount: purchaseType === "CREDITS" && !packCode ? creditAmount : undefined,
      pack_code: purchaseType === "CREDITS" ? packCode : undefined,
      coupon_code: coupon || undefined,
      payment_method: method,
    }),
    [billingPeriod, creditAmount, packCode, paymentMethod, planCode, purchaseType],
  );
  ...
  const loadPreview = useCallback(
    async (coupon?: string | null, method: PaymentMethod = paymentMethod, silent = false) => {
      ...
      if (purchaseType === "CREDITS" && !packCode && (!creditAmount || creditAmount <= 0)) {
        setError("Missing credit pack. Please go back to credits and choose a pack.");
        setLoading(false);
        return;
      }
    ...
    [billingPeriod, buildPayload, creditAmount, packCode, paymentMethod, planCode, purchaseType],
  );
    ...
    (async () => {
      try {
        ...
        if (initialCoupon) {
          ...
          await loadPreview(initialCoupon, defaultMethod, true);
        } else {
  ...
  const title = purchaseType === "SUBSCRIPTION" ? `${preview?.plan_code || planCode} Plan` : `${preview?.pack_title || preview?.pack_code || preview?.credit_amount || creditAmount} Credits`;
  const subtitle = purchaseType === "SUBSCRIPTION" ? `${preview?.billing_period || billingPeriod} billing` : (preview?.pack_code ? `Top-up pack ${preview.pack_code}` : "One-time credit top-up");
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-xl border border-white/10 bg-black/15 p-4">
                ...
                <p className="mt-2 text-2xl font-bold text-white">
                  {purchaseType === "SUBSCRIPTION" ? `${(preview?.plan?.included_credits || 0).toLocaleString()} credits` : `${(preview?.credit_amount || preview?.pack?.total_credits || creditAmount).toLocaleString()} credits`}
                </p>
                <p className="text-sm text-purple-100/60">{purchaseType === "SUBSCRIPTION" ? "Plan credits" : (Number(preview?.bonus_credits || 0) > 0 ? `${preview?.base_credits || 0} base + ${preview?.bonus_credits || 0} bonus` : "Wallet top-up")}</p>
              </div>
            </CardContent>
          </Card>
