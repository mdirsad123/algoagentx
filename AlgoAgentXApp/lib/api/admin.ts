...
import { HEAVY_API_TIMEOUT_MS, POLLING_API_TIMEOUT_MS } from "../api-timeouts";
...
export interface BillingSummary {
  ...
  total_gst_collected_inr: number;
  total_discounts_usd: number;
  total_paid_orders: number;
  pending_orders: number;
  failed_orders: number;
  coupon_redemptions: number;
  credit_topup_revenue_usd: number;
  subscription_revenue_usd: number;
}

export interface BillingAuditOrder {
  id: string;
  order_date: string;
  user_id?: string;
  user_email?: string;
...
export interface BillingAuditPayment {
  id: string;
  user_id?: string;
  user_email?: string;
  user_name?: string;
...
export interface StrategyRequest {
  ...
  assets?: StrategyAttachment[];
  strategy_assets?: StrategyAttachment[];
  strategyAssets?: StrategyAttachment[];
  ...
  original_strategy?: { id: string; name?: string | null; visibility?: string | null } | null;
  originalStrategy?: { id: string; name?: string | null; visibility?: string | null } | null;
  original_strategy_name?: string | null;
  originalStrategyName?: string | null;
  original_request?: { id: string; title?: string | null; status?: string | null } | null;
  originalRequest?: { id: string; title?: string | null; status?: string | null } | null;
...
export interface AdminStrategySandboxResult {
  ...
  summary: {
    ...
    rejection_setups?: number;
    confirmed_setups?: number;
    executed_trades?: number;
    rejected_setups?: number;
    strategy_rejection_reasons?: Record<string, number>;
    strategy_diagnostics?: Record<string, any>;
  };
  trades: Array<{
    entry_time?: string;
    exit_time?: string;
...
export interface ImplementedStrategy {
  ...
  workspace_status?: string | null;
  workspaceStatus?: string | null;
  source_request?: StrategyRequest | null;
  sourceRequest?: StrategyRequest | null;
}
...
export const duplicateAdminStrategyById = async (strategyId: string): Promise<ImplementedStrategy> =>
  unwrap(await axiosInstance.post(`/api/v1/admin/strategy-requests/strategies/${strategyId}/duplicate`));
...
export const adminApi = {
  ...
  getBillingSummary: async (): Promise<BillingSummary> =>
    unwrap(await axiosInstance.get("/api/v1/admin/billing/summary")),

  getBillingAuditOrders: async (params?: {
    page?: number; page_size?: number; status?: string; payment_method?: string; purchase_type?: string; coupon_code?: string; user_email?: string; from_date?: string; to_date?: string;
  }): Promise<PaginatedResponse<BillingAuditOrder>> =>
    unwrap(await axiosInstance.get("/api/v1/admin/billing/orders", { params })),

  getBillingAuditPayments: async (params?: {
    skip?: number; limit?: number; status?: string; payment_method?: string; purchase_type?: string; user_email?: string; from_date?: string; to_date?: string;
  }): Promise<PaginatedResponse<BillingAuditPayment>> =>
    unwrap(await axiosInstance.get("/api/v1/admin/billing/payments", { params })),
  ...
  getCreditPacks: async (): Promise<CreditTopupPack[]> =>
    unwrap(await axiosInstance.get("/api/v1/admin/credit-packs")),

  createCreditPack: async (payload: Partial<CreditTopupPack>): Promise<CreditTopupPack> =>
    unwrap(await axiosInstance.post("/api/v1/admin/credit-packs", payload)),

  updateCreditPack: async (id: string, payload: Partial<CreditTopupPack>): Promise<CreditTopupPack> =>
    unwrap(await axiosInstance.put(`/api/v1/admin/credit-packs/${id}`, payload)),

  updateCreditPackStatus: async (id: string, payload: { is_active?: boolean; is_popular?: boolean }): Promise<CreditTopupPack> =>
    unwrap(await axiosInstance.patch(`/api/v1/admin/credit-packs/${id}/status`, payload)),
  ...
  createStrategyWorkspace: async (requestId: string): Promise<{ request: StrategyRequest; strategy: ImplementedStrategy }> =>
    unwrap(await axiosInstance.post(`/api/v1/admin/strategy-requests/${requestId}/workspace`)),
  ...
  runAdminStrategySandboxBacktest: async (
    strategyId: string,
    payload: { instrument_id: number; timeframe: string; start_date: string; end_date: string; capital?: number; runtime_config?: Record<string, any> },
  ): Promise<AdminStrategySandboxResult> => {
    const queued = unwrap<any>(await axiosInstance.post(
      `/api/v1/admin/strategy-requests/strategies/${strategyId}/sandbox-backtest`,
      payload,
      { timeout: HEAVY_API_TIMEOUT_MS },
    ));

    if (!queued?.job_id || !["pending", "queued", "running"].includes(String(queued?.status || "").toLowerCase())) {
      return queued as AdminStrategySandboxResult;
    }

    for (;;) {
      await new Promise((resolve) => window.setTimeout(resolve, 3000));
      const jobResponse = await axiosInstance.get(`/api/v1/jobs/${queued.job_id}`, { timeout: POLLING_API_TIMEOUT_MS });
      const job = jobResponse.data || {};
      const status = String(job.status || "").toLowerCase();
      if (status === "completed") {
        return (job.result_data || {}) as AdminStrategySandboxResult;
      }
      if (status === "failed") {
        const errorMessage = job.result_data?.error || job.message || "Sandbox backtest failed";
  ...
  listAdminStrategyAssets: async (strategyId: string): Promise<{ items: StrategyAttachment[] }> =>
    unwrap(await axiosInstance.get(`/api/v1/admin/strategy-requests/strategies/${strategyId}/assets`)),

  uploadAdminStrategyAssets: async (strategyId: string, files: File[], options?: { captions?: string[]; is_public?: boolean }): Promise<{ items: StrategyAttachment[] }> => {
    const formData = new FormData();
    files.forEach((file) => formData.append("files", file));
    (options?.captions || []).forEach((caption) => formData.append("captions", caption));
    if (typeof options?.is_public === "boolean") formData.append("is_public", String(options.is_public));
    return unwrap(await axiosInstance.post(`/api/v1/admin/strategy-requests/strategies/${strategyId}/assets`, formData));
  },

  updateAdminStrategyAsset: async (strategyId: string, assetId: string, payload: Partial<StrategyAttachment>): Promise<StrategyAttachment> =>
    unwrap(await axiosInstance.patch(`/api/v1/admin/strategy-requests/strategies/${strategyId}/assets/${assetId}`, payload)),

  deleteAdminStrategyAsset: async (strategyId: string, assetId: string): Promise<{ id: string }> =>
    unwrap(await axiosInstance.delete(`/api/v1/admin/strategy-requests/strategies/${strategyId}/assets/${assetId}`)),

  reorderAdminStrategyAssets: async (strategyId: string, items: { id: string; sort_order: number }[]): Promise<{ items: StrategyAttachment[] }> =>
    unwrap(await axiosInstance.post(`/api/v1/admin/strategy-requests/strategies/${strategyId}/assets/reorder`, { items })),
  ...
  deleteAdminStrategyById: async (strategyId: string): Promise<{
    id: string;
    deletion_mode: "DELETED" | "ARCHIVED";
    archived: boolean;
    released_request_count: number;
    preserved_references?: {
      normal_backtests?: number;
      funded_backtests?: number;
      deployments?: number;
  ...
  deployPrivateAdminStrategyById: async (strategyId: string): Promise<ImplementedStrategy> =>
    unwrap(await axiosInstance.post(`/api/v1/admin/strategy-requests/strategies/${strategyId}/deploy-private`)),
