...
export function RuntimeSettingsDrawer(props: Props) {
  ...
  return <div className="fixed inset-0 z-[120] flex items-stretch justify-end bg-black/55 backdrop-blur-sm">
    ...
    <aside className="relative z-[121] flex h-[100dvh] w-full max-w-[min(48rem,calc(100vw-0.75rem))] flex-col overflow-hidden border-l border-border/60 bg-[#211042]/95 shadow-2xl backdrop-blur-xl sm:max-w-3xl">
      <div className="flex items-start justify-between gap-3 border-b border-border/50 px-3 py-3 sm:px-5 sm:py-4">
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold text-foreground sm:text-xl">{props.title || "Strategy Runtime Settings"}</h2>{props.badgeLabel && <Badge className="border-primary/50 bg-primary/20 text-primary-foreground">{props.badgeLabel}</Badge>}</div><p className="mt-1 text-sm text-muted-foreground">{props.subtitle || "Configure risk, SL/TP, execution and dynamic strategy inputs."}</p></div>
        <Button type="button" variant="ghost" size="sm" className="rounded-xl" onClick={props.onClose}><X className="h-5 w-5" /></Button>
      </div>
      <div className="grid grid-cols-1 gap-2 border-b border-border/50 px-3 py-3 min-[430px]:grid-cols-2 sm:gap-3 sm:px-5 sm:py-4 md:grid-cols-5">
      ...
      <div className="flex gap-2 overflow-x-auto border-b border-border/50 px-3 py-3 sm:flex-wrap sm:px-5">{RUNTIME_TABS.map((tab) => <button key={tab.value} type="button" onClick={() => props.onTabChange(tab.value)} className={`shrink-0 rounded-xl border px-3 py-2 text-xs font-medium transition ${props.activeTab === tab.value ? "border-primary/60 bg-primary/20 text-primary-foreground" : "border-border/60 bg-card/20 text-muted-foreground hover:bg-card/40 hover:text-foreground"}`}>{tab.label}</button>)}</div>
      <div className="flex-1 overflow-y-auto px-3 py-4 sm:px-5 sm:py-5"><RuntimeSettingsForm activeTab={props.activeTab} config={props.config} updateSection={props.updateSection} strategySchema={props.strategySchema} updateStrategyParam={props.updateStrategyParam} initialCapital={props.initialCapital} onInitialCapitalChange={props.onInitialCapitalChange} currency={props.currency} instrumentSymbol={props.instrumentSymbol} supportsIntradaySquareOff={props.supportsIntradaySquareOff} /></div>
      <div className="flex flex-col gap-3 border-t border-border/50 px-3 py-3 sm:px-5 sm:py-4 md:flex-row md:items-center md:justify-between"><p className="text-xs text-muted-foreground">{props.footerNote || "Settings are sent with the next request only."}</p><div className="flex flex-col gap-2 min-[430px]:flex-row min-[430px]:flex-wrap">{props.onReset && <Button type="button" variant="outline" onClick={props.onReset} className="rounded-xl border-border/60 bg-card/20 text-foreground hover:bg-card/40">Reset to Defaults</Button>}<Button type="button" disabled={props.valid === false} onClick={props.onApply || props.onClose} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">Apply Settings</Button></div></div>
    </aside>
  </div>;
}
